...
interface Booking {
    ...
    schedule_end: string;
    status: string;
    meeting_location: string | { address?: string; notes?: string; [key: string]: any } | null;
    service_price: {
    ...
    <AuthenticatedLayout>
        ...
        <div class="max-w-6xl mx-auto pb-24">
            ...
            <div class="flex flex-col sm:flex-row sm:items-end justify-between gap-6 mb-12 mt-4">
                <div>
                    <h1 class="text-4xl font-black text-gray-900 tracking-tight mb-2">Sesi Tatap Muka</h1>
                    <p class="text-gray-500 font-medium text-lg">Kelola jadwal pertemuan tatap muka Anda dengan mudah.</p>
                </div>
                <Link :href="route('client.bookings.create')" class="inline-flex items-center justify-center gap-2 px-8 py-3.5 bg-emerald-600 text-white font-bold rounded-full hover:bg-emerald-700 transition-all shadow-[0_8px_20px_rgba(16,185,129,0.25)] hover:shadow-[0_12px_25px_rgba(16,185,129,0.35)] hover:-translate-y-1">
            ...
            <div class="mb-16">
                ...
                <div v-if="activeBookings.length > 0" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    <div v-for="booking in activeBookings" :key="booking.id" class="bg-white rounded-3xl border border-gray-100 shadow-[0_4px_20px_rgb(0,0,0,0.03)] overflow-hidden flex flex-col hover:shadow-[0_10px_30px_rgb(0,0,0,0.08)] transition-all hover:-translate-y-1 hover:border-emerald-200">
                        ...
                        <div class="bg-gray-50/50 border-b border-gray-100 px-6 py-4 flex items-center justify-between">
                            <div class="flex items-center gap-2 text-gray-500 font-bold text-xs sm:text-sm">
                                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" class="text-emerald-500"><rect width="18" height="18" x="3" y="4" rx="2" ry="2"/><line x1="16" x2="16" y1="2" y2="6"/><line x1="8" x2="8" y1="2" y2="6"/></svg>
                        ...
                        <div class="p-6 flex-1 flex flex-col items-center text-center relative">
                            ...
                            <div class="absolute top-10 left-1/2 -translate-x-1/2 w-24 h-24 bg-emerald-100 rounded-full blur-2xl pointer-events-none"></div>
                            ...
                            <p class="text-xs font-bold text-emerald-600 uppercase tracking-widest relative z-10">{{ booking.service_price.duration_minutes }} Menit Sesi</p>
                        </div>

                        <!-- Full Width Join Button / Location Info -->
                        <div class="px-6 pb-6 mt-auto">
                            <Link v-if="getLocationString(booking.meeting_location)" :href="route('client.bookings.show', booking.id)" 
                                class="w-full py-4 bg-emerald-600 hover:bg-emerald-700 text-white font-black rounded-2xl flex items-center justify-center gap-2 transition-all shadow-[0_4px_15px_rgba(16,185,129,0.25)] hover:shadow-[0_8px_20px_rgba(16,185,129,0.35)]">
                ...
                <div v-else class="bg-gray-50 border-2 border-dashed border-gray-200 rounded-[2rem] p-12 text-center">
                    ...
                    <p class="text-gray-500">Jadwalkan sesi pertemuan tatap muka pertama Anda sekarang.</p>
                </div>
            </div>
            ...
            <div>
                ...
                <div v-else class="flex flex-col gap-4">
                    <Link v-for="booking in pastBookings" :key="booking.id" :href="route('client.bookings.show', booking.id)" 
                            class="bg-white rounded-3xl p-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6 border border-gray-100 shadow-sm hover:shadow-md hover:border-emerald-200 transition-all group">
                        ...
                        <div class="flex items-center gap-5 w-full sm:w-1/3">
                            ...
                            <div>
                                <h4 class="font-bold text-gray-900 text-lg group-hover:text-emerald-600 transition-colors">{{ booking.counselor.name }}</h4>
                        ...
                        <div class="flex items-center justify-end w-full sm:w-1/3">
                            <div class="hidden sm:flex items-center gap-3 text-gray-400 group-hover:text-emerald-600 font-bold text-sm transition-colors">
                                Lihat Detail
                                <div class="w-10 h-10 rounded-full bg-gray-50 border border-gray-100 flex items-center justify-center group-hover:bg-emerald-50 group-hover:border-emerald-200 transition-all">
