        ...
        <div class="max-w-6xl mx-auto pb-24">
            ...
            <div class="flex flex-col sm:flex-row sm:items-end justify-between gap-6 mb-12 mt-4">
                <div>
                    ...
                    <p class="text-gray-500 font-medium text-lg">Kelola jadwal pertemuan tatap muka Anda dengan klien.</p>
                </div>
            </div>
            ...
            <div class="mb-16">
                ...
                <div v-if="activeBookings.length > 0" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    <div v-for="booking in activeBookings" :key="booking.id" class="bg-white rounded-3xl border border-gray-100 shadow-[0_4px_20px_rgb(0,0,0,0.03)] overflow-hidden flex flex-col hover:shadow-[0_10px_30px_rgb(0,0,0,0.08)] transition-all hover:-translate-y-1 hover:border-emerald-200">
                        ...
                        <div class="p-6 flex-1 flex flex-col items-center text-center relative">
                            ...
                            <div class="relative w-24 h-24 rounded-full mb-5 shadow-sm border-4 border-white z-10 flex items-center justify-center bg-emerald-600 text-white text-3xl font-black">
                        ...
                        <!-- Full Width Join Button / Location Info / Detail Button -->
                        <div class="px-6 pb-6 mt-auto flex flex-col gap-3">
                            <Link :href="route('counselor.bookings.show', booking.id)" 
                                :class="[
                                    ...
                                    getLocationString(booking.meeting_location)
                                        ? 'bg-emerald-600 hover:bg-emerald-700 text-white shadow-[0_4px_15px_rgba(16,185,129,0.25)] hover:shadow-[0_8px_20px_rgba(16,185,129,0.35)]' 
                                        : 'bg-emerald-600 hover:bg-emerald-700 text-white shadow-[0_4px_15px_rgba(16,185,129,0.25)] hover:shadow-[0_8px_20px_rgba(16,185,129,0.35)]'
                                ]">
                                ...
                                {{ getLocationString(booking.meeting_location) ? 'Lihat / Ubah Lokasi' : 'Atur Lokasi Pertemuan' }}
                            </Link>
                        </div>
                    </div>
                </div>
                ...
                <div v-else class="bg-gray-50 border-2 border-dashed border-gray-200 rounded-[2rem] p-12 text-center">
                    ...
                    <p class="text-gray-500">Belum ada jadwal sesi tatap muka mendatang dengan klien.</p>
                </div>
            </div>
                ...
                <div v-else class="flex flex-col gap-4">
                    <Link v-for="booking in pastBookings" :key="booking.id" :href="route('counselor.bookings.show', booking.id)" 
                        ...
                        <div class="flex items-center gap-5 w-full sm:w-1/3">
                            <div class="w-16 h-16 rounded-full flex items-center justify-center text-white bg-emerald-600 font-black text-xl grayscale opacity-70 group-hover:grayscale-0 group-hover:opacity-100 transition-all shrink-0">
                                {{ getInitials(booking.client.name) }}
                            </div>
                            <div>
                                <h4 class="font-bold text-gray-900 text-lg group-hover:text-emerald-600 transition-colors">{{ booking.client.name }}</h4>
